...
def test_parse_script_valid() -> None:
    ...
    assert script.defaults["voice_rate"] == 168


def test_parse_script_rejects_cyrillic_vo() -> None:
    with pytest.raises(ValueError, match="English-only"):
        parse_script(
            {
                "title": "x",
                "segments": [{"vo": "Сезон пятьдесят восемь. Вилка.", "caption": "DOLOS"}],
            }
        )


def test_parse_script_scrubs_github_and_crypto_from_vo() -> None:
    from helios.calliope import scrub_broad_vo

    cleaned = scrub_broad_vo(
        "Star us on GitHub. github.com slash alexar76 slash dolos. Pay in USDC on Base settlement."
    )
    assert "github" not in cleaned.lower()
    assert "usdc" not in cleaned.lower()
    assert "Open source. MIT." in cleaned
    assert "Crypto stays off this cut." in cleaned

    script = parse_script(
        {
            "title": "DOLOS soft star",
            "repos": ["dolos"],
            "segments": [
                {
                    "vo": "Try it live. github.com slash alexar76 slash dolos. Soft star.",
                    "caption": "SOFT STAR",
                    "visual": {"type": "image", "path": "alien-monitor-hero.gif"},
                },
                {
                    "vo": "Connect your wallet and settle in USDC.",
                    "caption": "CAPABILITY",
                    "visual": {"type": "image", "path": "alien-monitor-hero.gif"},
                },
            ],
    ...
    joined = " ".join(s["vo"] for s in script.segments).lower()
    assert "github" not in joined
    assert "usdc" not in joined
    assert "connect your wallet" not in joined
