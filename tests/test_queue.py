...
def test_worker_lock_reclaims_stale_pid(store: QueueStore) -> None:
    store._lock_path.write_text("999999999", encoding="utf-8")
...
def test_worker_lock_permission_denied_returns_false(store: QueueStore, monkeypatch) -> None:
    def _deny(*_a, **_k):
        raise PermissionError("read-only data dir")

    monkeypatch.setattr("helios.queue.os.open", _deny)
    assert store.acquire_lock() is False
