...
def test_prioritize_creator_before_backfill_release_last(tmp_path: Path) -> None:
    ...
    jobs = [
        ...
        Job(id="j3", idempotency_key="c", status="pending", template="calliope-episode", phase="creator", created_at="2026-01-03"),
        Job(id="j4", idempotency_key="d", status="pending", template="theoros-short", phase="creator", created_at="2026-01-04"),
    ]
    ordered = director.prioritize(jobs)
    assert ordered[0].template == "theoros-short"
    assert ordered[1].template == "calliope-episode"
    assert ordered[2].phase == "backfill"
    assert ordered[-1].template == "release-short"
