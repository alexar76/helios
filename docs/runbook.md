...
## Visuals (do not put Factory UI on Platon)

`helios/visuals_catalog.py` maps each repo → hero/still under `assets/`.

| Repo / topic | Asset |
|---|---|
| `platon`, `oracles`, VRF/chaos | `platon-umbral-hero.gif` / `platon-cosmos.png` |
| Factory / hub / argus | `hero-demo-preview.gif` |
| alien-monitor / metis | `alien-monitor-hero.gif` |
| courses | `course-hero-16x9.png` |

Every render **coerces** wrong paths (Calliope used to hard-code the Factory GIF onto oracle shorts). After changing assets:

```bash
# host
rsync -a helios/assets/ <host>:/path/to/helios/assets/
docker exec helios helios worker --max-jobs 1   # re-render pending
```

Private YouTube drafts with the wrong visual must be **re-rendered** (new job) — uploads already public stay wrong until replaced.
