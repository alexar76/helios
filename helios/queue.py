...
class QueueStore:
    def __init__(self, data_dir: Path, *, file_roots: list[Path] | None = None) -> None:
        ...
        self._queue_lock_path = data_dir / ".queue.lock"

    @contextmanager
    def _queue_lock(self):
        """Cross-process exclusive lock for all queue.json mutations."""
        self._queue_lock_path.parent.mkdir(parents=True, exist_ok=True)
        with open(self._queue_lock_path, "a", encoding="utf-8") as lock_file:
    ...
    def _clear_stale_worker_lock(self, *, max_age_sec: int = 7200) -> None:
        ...
        try:
            ...
            return
        try:
            age = datetime.now(timezone.utc).timestamp() - self._lock_path.stat().st_mtime
            if age > max_age_sec:
                self._lock_path.unlink(missing_ok=True)
        except OSError:
            pass
        ...
        with self._queue_lock():
            ...
            for j in jobs:
                if j.get("status") != "rendering":
                    continue
                try:
                    ts = datetime.fromisoformat(str(j.get("updated_at", "")).replace("Z", "+00:00")).timestamp()
                except ValueError:
                    ts = 0
                if ts < cutoff:
                    j["status"] = "pending"
    ...
    def acquire_lock(self) -> bool:
        self._clear_stale_worker_lock()
        try:
            fd = os.open(str(self._lock_path), os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600)
        except FileExistsError:
            return False
        except PermissionError:
            # Read-only /data or wrong mount ownership — do not crash the worker loop.
            return False
        try:
