...
from helios.worker import approve_job, run_worker, unpublish_job
...
@app.command()
def unpublish(job_id: str) -> None:
    """public → private (hide a bad upload)."""
    cfg = load_config()
    job = unpublish_job(cfg, job_id, actor="operator")
    typer.echo(f"Unpublished (private): https://youtu.be/{job.video_id}")
...
@calliope_app.command("fetch")
def calliope_fetch(
    repo: Optional[str] = typer.Option(None, "--repo", "-r", help="Single repo (else season/allowlist)"),
) -> None:
    """Fetch README + latest release from GitHub into data/fetch/ for Calliope."""
    cfg = load_config()
    store = QueueStore(cfg.data_dir, file_roots=file_roots(cfg))
    muse = Calliope(cfg, store)
    repos = [repo] if repo else None
    result = muse.refresh_github(repos=repos)
    typer.echo(json.dumps(result, indent=2))


@calliope_app.command("season")
def calliope_season() -> None:
    """Show the active content season (themes + focus repos)."""
    cfg = load_config()
    store = QueueStore(cfg.data_dir, file_roots=file_roots(cfg))
    muse = Calliope(cfg, store)
    s = muse.active_season()
    if not s:
        typer.echo("{}", err=True)
        raise typer.Exit(1)
    typer.echo(json.dumps({
...
def serve(port: Optional[int] = typer.Option(None, "--port")) -> None:
    ...
    def snapshot() -> dict:
        return {
            ...
            "uploaded_today": store.daily_upload_count(),
