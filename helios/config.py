...
class HeliosConfig:
    ...
    github_fetch_enabled: bool = True
    seasons_raw: list[dict[str, Any]] = field(default_factory=list)
    accept_release_shorts: bool = False
...
def load_config(config_path: Path | None = None) -> HeliosConfig:
    ...
    # Release "shipped" shorts — opt-in (seasonal Calliope + Theoros are the main mix).
    accept_rel = _env("HELIOS_RELEASE_SHORTS", "").strip().lower()
    if accept_rel in ("1", "true", "yes"):
        accept_release_shorts = True
    elif accept_rel in ("0", "false", "no"):
        accept_release_shorts = False
    else:
        accept_release_shorts = bool(raw.get("accept_release_shorts", False))

    seasons_raw = raw.get("seasons") if isinstance(raw.get("seasons"), list) else []
    github_fetch_enabled = bool(github.get("fetch_enabled", True))
    ...
    return HeliosConfig(
        ...
        github_fetch_enabled=github_fetch_enabled,
        seasons_raw=[s for s in seasons_raw if isinstance(s, dict)],
        accept_release_shorts=accept_release_shorts,
        raw=raw,
    )
...
def config_summary(cfg: HeliosConfig) -> dict[str, Any]:
    return {
        ...
        "github_fetch": cfg.github_fetch_enabled,
        "seasons": len(cfg.seasons_raw) or "defaults",
