...
- English titles/descriptions on the broad channel. Reject Cyrillic clickbait.
- Broad track: no crypto hype in titles (USDC / Base wallet flex). Crypto-room pieces stay fenced.
...
class Director:
    ...
    def prioritize(self, jobs: list[Job]) -> list[Job]:
        """Order pending jobs: creator (Calliope/Theoros) → backfill → release-shorts last."""
        creator = [j for j in jobs if j.phase == "creator"]
        backfill = [j for j in jobs if j.phase == "backfill"]
        release = [j for j in jobs if j.template == "release-short"]
        steady = [
            j for j in jobs
            if j.phase not in ("backfill", "creator") and j.template != "release-short"
        ]
        ...
        # Theoros + Calliope before PromoMaterials backlog; release spam last.
        theoros = [j for j in creator if j.template == "theoros-short"]
        other_creator = [j for j in creator if j.template != "theoros-short"]
        theoros.sort(key=lambda j: j.created_at)
        other_creator.sort(key=lambda j: j.created_at)
        backfill.sort(key=episode_key)
        steady.sort(key=lambda j: j.created_at)
        release.sort(key=lambda j: j.created_at)
        return theoros + other_creator + backfill + steady + release
