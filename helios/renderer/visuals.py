...
def make_card_image(visual: dict[str, Any], out_png: Path) -> None:
    ...
    from helios.renderer.still_pipeline import text_safe_rect, wrap_text
    ...
    x0, y0, max_w, max_h = text_safe_rect((fmt.W, fmt.H))
    raw = [ln for ln in str(text).split("\n") if ln.strip()] or ["HELIOS"]
    lines: list[str] = []
    for ln in raw:
        lines.extend(wrap_text(draw, ln, font, max_w))
    ...
    for line in lines:
        ...
        cx = x0 + max(0, (max_w - tw) // 2)
        draw.text((cx + 2, y + 2), line, fill=(0, 0, 0), font=font)
        draw.text((cx, y), line, fill=(230, 233, 242), font=font)
        y += line_h
    img.save(out_png)
