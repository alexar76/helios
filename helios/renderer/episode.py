"""Core episode build — TTS + ffmpeg + subtitles."""

from __future__ import annotations

import json
import shutil
import subprocess
from pathlib import Path
from typing import Any

from helios.config import HeliosConfig
from helios.renderer import format as fmt
from helios.renderer import tts as tts_mod
from helios.renderer import visuals as vis_mod
from helios.renderer.subtitles import build_srt, overlay_caption
from helios.security import resolve_asset_path, safe_subprocess_arg, substitute_vars
from helios.visuals_catalog import coerce_asset_for_repo, primary_visual, visuals_for_repo


def _run(cmd: list[str]) -> None:
    safe = [safe_subprocess_arg(str(c)) for c in cmd]
    subprocess.run(safe, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)


def _mux_av(video: Path, audio: Path, out: Path) -> None:
    _run([
        "ffmpeg", "-y", "-i", str(video), "-i", str(audio),
        "-c:v", "copy", "-c:a", "aac", "-b:a", "192k", "-shortest", str(out),
    ])


def _concat(parts: list[Path], out: Path) -> None:
    lst = out.with_suffix(".txt")
    with lst.open("w") as f:
        for p in parts:
            f.write(f"file '{p}'\n")
    _run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(lst), "-c", "copy", str(out)])
    lst.unlink(missing_ok=True)


def _fill_visual_vars(vars: dict[str, str]) -> dict[str, str]:
    """Ensure hero/still match the repo — kills Factory-GIF-on-Platon / course-on-everything."""
    out = dict(vars)
    repo = out.get("repo", "")
    topic = out.get("topic", "")
    hero, still = visuals_for_repo(repo)
    if not normalize_repo_safe(repo) and topic:
        hero = still = primary_visual("", topic)
    for key, fallback in (("hero", hero), ("still", still)):
        cur = (out.get(key) or "").strip()
        if (
            not cur
            or cur.startswith("{")
            or cur.endswith("course-hero-16x9.png")
            or cur.endswith("hero-demo-preview.gif")
        ):
            out[key] = fallback
        else:
            out[key] = coerce_asset_for_repo(cur, repo, topic)
    return out


def normalize_repo_safe(repo: str) -> str:
    return (repo or "").strip().split("/")[-1].lower()


def build_from_template(
    cfg: HeliosConfig,
    template: dict[str, Any],
    vars: dict[str, str],
    out_dir: Path,
) -> dict[str, Path]:
    defaults = template.get("defaults", {})
    voice = defaults.get("voice", "Daniel")
    rate = int(defaults.get("voice_rate", 168))
    # Dual layout: 16:9 landscape or native 9:16 shorts (uses *-shorts.png diagrams).
    fmt.set_format(str(defaults.get("aspect", "16:9")))
    work = out_dir / ".work"
    if work.exists():
        shutil.rmtree(work)
    work.mkdir(parents=True)

    vars = _fill_visual_vars(vars)

    parts: list[Path] = []
    srt_entries: list[tuple[float, float, str]] = []
    t_cursor = 0.0

    def _resolve(rel: str) -> Path:
        return resolve_asset_path(rel, cfg.asset_roots)

    for idx, seg in enumerate(template.get("segments", []), 1):
        vo = substitute_vars(seg["vo"].strip(), vars)
        caption = substitute_vars(seg.get("caption", vo).strip().upper(), vars)
        visual = dict(seg.get("visual", {}))
        if "path_shorts" in visual:
            visual["path_shorts"] = substitute_vars(str(visual["path_shorts"]), vars)
        if "path" in visual:
            rel = substitute_vars(visual["path"], vars)
            # ALWAYS remap wrong product visuals (Calliope used to hard-code Factory GIF
            # onto Platon/oracles — coerce every template, not only release-short).
            if "{" in rel:
                rel = rel.replace("{hero}", vars.get("hero", "")).replace("{still}", vars.get("still", ""))
            rel = coerce_asset_for_repo(rel, vars.get("repo", ""), vars.get("topic", ""))
            basename = Path(rel).name
            if basename.endswith(".gif"):
                visual["type"] = "image"
                visual["loop"] = True
            visual["path"] = str(_resolve(basename))
            # Prefer stacked *-shorts.png when rendering 9:16 release-shorts.
            chosen = fmt.resolve_visual_asset(
                {"path": visual["path"], "path_shorts": visual.get("path_shorts")},
                resolve_rel=lambda r: str(_resolve(r)),
            )
            if chosen:
                visual["path"] = chosen
        if visual.get("type") == "card" and "text" in visual:
            visual["text"] = substitute_vars(visual["text"], vars)

        wav = work / f"{idx:02d}.wav"
        dur = tts_mod.tts_wav(vo, voice, rate, wav) + 0.25
        vis = work / f"{idx:02d}_v.mp4"
        vis_mod.make_visual(visual, dur, vis)
        mux = work / f"{idx:02d}_mux.mp4"
        _mux_av(vis, wav, mux)
        seg_out = work / f"{idx:02d}_seg.mp4"
        overlay_caption(mux, caption, seg_out)
        parts.append(seg_out)
        srt_entries.append((t_cursor, t_cursor + dur, caption))
        t_cursor += dur

    raw = work / "raw.mp4"
    _concat(parts, raw)
    video = out_dir / "video.mp4"
    shutil.copy2(raw, video)
    srt = out_dir / "video.srt"
    srt.write_text(build_srt(srt_entries), encoding="utf-8")
    meta = {
        "duration": round(t_cursor, 2),
        "template": template.get("id"),
        "aspect": "9:16" if fmt.FORMAT == "shorts" else "16:9",
        "format": fmt.FORMAT,
        "hero": vars.get("hero"),
        "still": vars.get("still"),
        "repo": vars.get("repo"),
    }
    (out_dir / "video.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")
    shutil.rmtree(work, ignore_errors=True)
    return {"video": video, "srt": srt}
