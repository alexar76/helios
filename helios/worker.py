...
def _video_luma_qc(video_path: Path, *, samples: int = 3) -> dict[str, Any]:
    """Reject near-black renders (e.g. missing TTF → invisible card text)."""
    import json
    import subprocess
    ...
    duration = float(stream.get("duration") or 0) or 1.0
    is_shorts = height > width

    dark_fracs: list[float] = []
    light_fracs: list[float] = []
    means: list[float] = []
    for i in range(samples):
        t = max(0.2, duration * (i + 1) / (samples + 1))
        frame = video_path.with_name(f".qc_{video_path.stem}_{i}.png")
        try:
            subprocess.run(
                ["ffmpeg", "-y", "-ss", f"{t:.2f}", "-i", str(video_path), "-frames:v", "1", str(frame)],
            ...
            im = im.resize((160, max(1, int(160 * im.height / im.width))))
            px = list(im.get_flattened_data()) if hasattr(im, "get_flattened_data") else list(im.getdata())
            n = len(px) or 1
            dark = sum(1 for r, g, b in px if (r + g + b) < 40) / n
            light = sum(1 for r, g, b in px if (r + g + b) > 400) / n
            mean = sum(r + g + b for r, g, b in px) / (3 * n)
            dark_fracs.append(dark)
            light_fracs.append(light)
            means.append(mean)
        finally:
            if frame.exists():
                frame.unlink(missing_ok=True)

    avg_dark = sum(dark_fracs) / max(1, len(dark_fracs))
    avg_light = sum(light_fracs) / max(1, len(light_fracs))
    avg_mean = sum(means) / max(1, len(means))
    # Brand cards are intentionally near-black; after yuv420p encode dark_frac is high.
    # Fail only when there is essentially no light text/caption signal.
    # 0.0015 leaves margin for shorts (1080x1920) where light text is a thin caption band.
    ok = avg_light >= 0.0015
    reason = ""
    if not ok:
        reason = (
            f"black_frame_qc: light_frac={avg_light:.4f} dark_frac={avg_dark:.2f} "
            f"mean_luma={avg_mean:.1f} ({width}x{height}) — refusing upload"
        )
    return {
        "ok": ok,
        "reason": reason,
        "dark_frac": round(avg_dark, 3),
        "light_frac": round(avg_light, 4),
        "mean_luma": round(avg_mean, 1),
        "width": width,
        "height": height,
        "is_shorts": is_shorts,
    }
...
def process_job(cfg: HeliosConfig, store: QueueStore, job: Job, audit: AuditLog, director: Director) -> None:
    ...
    qc = _video_luma_qc(video_path)
    if not qc["ok"]:
        job.status = "failed"
        job.error = qc["reason"]
        store.update_job(job)
        audit.append("render.qc_fail", job_id=job.id, data=qc)
        raise RuntimeError(qc["reason"])
    ...
    # YouTube Shorts shelf needs #Shorts in title for vertical renders.
    title = str(job.youtube.get("title") or "")
    if qc.get("is_shorts") and "#Shorts" not in title and "#shorts" not in title:
        job.youtube["title"] = (title[:90].rstrip() + " #Shorts")[:100]
        store.update_job(job)
...
def run_worker(cfg: HeliosConfig | None = None, *, max_jobs: int | None = None) -> int:
    ...
    try:
        store.recover_stale()
        ingested = ingest_external_queue(
            store,
            audit,
            file_roots=file_roots(cfg),
            accept_release_shorts=cfg.accept_release_shorts,
        )
        if ingested:
...
def unpublish_job(cfg: HeliosConfig, job_id: str, *, actor: str = "operator") -> Job:
    """public/unlisted → private (hide a bad auto-approve)."""
    store = QueueStore(cfg.data_dir, file_roots=file_roots(cfg))
    audit = AuditLog(cfg.data_dir / "audit.jsonl")
    job = store.get_job(job_id)
    if not job:
        raise ValueError(f"job not found: {job_id}")
    if not job.video_id:
        raise ValueError(f"job has no video_id: {job_id}")
    if not cfg.youtube_client_secret or not cfg.youtube_token:
        raise FileNotFoundError("YOUTUBE_CLIENT_SECRET and YOUTUBE_TOKEN required")
    youtube = get_youtube(client_secret=cfg.youtube_client_secret, token_path=cfg.youtube_token)
    publish_video(youtube, job.video_id, "private")
    job.youtube["privacy"] = "private"
    job.status = "awaiting_approval"
    store.update_job(job)
    audit.append("youtube.unpublished", job_id=job.id, actor=actor, data={"video_id": job.video_id})
    return job
