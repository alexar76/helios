...
def ingest_external_queue(
    ...
    *,
    ...
    accept_release_shorts: bool | None = None,
) -> int:
    ...
    if accept_release_shorts is None:
        accept_release = os.environ.get("HELIOS_RELEASE_SHORTS", "").strip().lower() in (
            "1", "true", "yes",
        )
    else:
        accept_release = accept_release_shorts
    ...
    with qfile:
        ...
        for line in lines:
            ...
            try:
                ...
                template = str(payload.get("template", "release-short"))
                if template == "release-short" and not accept_release:
                    audit.append(
                        "ingest.skipped",
                        data={"template": template, "reason": "release_shorts_disabled"},
                    )
                    count += 1  # consume the line
                    continue
