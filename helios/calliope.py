...
Tone: clear, technical-but-accessible, MIT open-source, no hype/clickbait.
Language: English VO and English on-screen captions ONLY — never Russian/Cyrillic,
Spanish, French, or Chinese in spoken narration or captions.
...
- Mention live demo hostnames when corpus has them (e.g. play.modelmarket.dev) — never invent URLs.
- Broad-track charter (non-negotiable, matches PromoMaterials seasons):
  - Do NOT speak GitHub URLs or "github.com slash …" / "star us on GitHub". Soft-star /
    open-source / MIT is fine; the spoken path stays off-camera.
  - Crypto OFF in VO: no USDC, Base settlement flex, "connect your wallet", or chain demos.
    Film the capability and the receipt. Wallet stays optional and unspoken on broad.
  - End with a soft open-source CTA (MIT / soft star / open the live demo) — not a repo URL.
"""
...
MAX_VO_PER_SEG = 400
MAX_CAPTION = 80
DEFAULT_VOICE_RATE = 168  # PromoMaterials ecosystem seasons (Daniel)

# Broad-track VO guards — same charter as PromoMaterials seasons.
_CYRILLIC_RE = re.compile(r"[\u0400-\u04FF]")
_GITHUB_VO_RE = re.compile(
    r"(?i)\b(?:star\s+(?:us\s+)?on\s+)?github(?:\.com)?(?:\s+slash\s+[\w./-]+|"
    r"(?:\s*/\s*|\s+)[\w./-]+)?\b"
)
_CRYPTO_VO_RE = re.compile(
    r"(?i)\b(?:USDC|USDT|ETH\b|on[- ]chain\s+wallet|connect\s+your\s+wallet|"
    r"Base\s+settlement|wallet\s+flex|mint\s+on\s+Base)\b"
)
_SOFT_OPEN_SOURCE = "Open source. MIT."
_SOFT_CRYPTO_OFF = "Crypto stays off this cut."


def _assert_english_text(text: str, *, field: str) -> None:
    if _CYRILLIC_RE.search(text or ""):
        raise ValueError(f"{field} must be English-only (Cyrillic found)")


def scrub_broad_vo(text: str) -> str:
    """Strip spoken GitHub paths and crypto flex from broad-track VO."""
    out = _GITHUB_VO_RE.sub(_SOFT_OPEN_SOURCE, text or "")
    out = _CRYPTO_VO_RE.sub(_SOFT_CRYPTO_OFF, out)
    # Collapse accidental double soft CTAs after overlapping rewrites.
    out = re.sub(r"(?:Open source\. MIT\.\s*){2,}", f"{_SOFT_OPEN_SOURCE} ", out)
...
def _validate_segment(
    ...
    *,
    ...
    vo = scrub_broad_vo(sanitize_text(str(seg.get("vo", "")), max_len=MAX_VO_PER_SEG))
    raw_caption = scrub_broad_vo(
        sanitize_text(str(seg.get("caption", vo[:MAX_CAPTION])), max_len=MAX_CAPTION)
    )
    caption = raw_caption.upper()
    if not vo:
        raise ValueError("segment missing vo")
    _assert_english_text(vo, field="vo")
    _assert_english_text(caption, field="caption")
...
def parse_script(data: dict[str, Any]) -> EpisodeScript:
    ...
    defaults: dict[str, Any] = {
        ...
        "voice_rate": int(raw_defaults.get("voice_rate", DEFAULT_VOICE_RATE) or DEFAULT_VOICE_RATE),
