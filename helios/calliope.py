...
- Allowed image/gif paths (must pick from this list):
  - course-hero-16x9.png
  - hero-demo-preview.gif
  - alien-monitor-hero.gif
  - aicom-landing-hero.gif
- Example visual: {"type":"image","path":"hero-demo-preview.gif","kenburns":true}
...
# Fallback product visuals when the LLM still emits black cards.
# Prefer still PNGs — GIFs need stream_loop and often fail under -loop 1.
_CALLIOPE_VISUAL_POOL = (
    "course-hero-16x9.png",
)
...
def _validate_segment(seg: dict[str, Any], *, index: int = 0) -> dict[str, Any]:
    ...
    if vtype not in ("card", "image", "video"):
        vtype = "image"
    # Always cycle product visuals — LLM otherwise repeats the same gif/card.
    asset = _CALLIOPE_VISUAL_POOL[index % len(_CALLIOPE_VISUAL_POOL)]
    if vtype == "card" or not str(visual.get("path", "")).strip():
        safe_visual = {
            "type": "image",
            "path": asset,
            "kenburns": True,
            **({"loop": True} if asset.endswith(".gif") else {}),
        }
    else:
        rel = str(visual.get("path", asset))
        if ".." in rel or rel.startswith("/"):
            rel = asset
        rel = Path(rel).name
        if rel not in _CALLIOPE_VISUAL_POOL:
            rel = asset
        # Prefer the indexed pool asset so segments differ on screen.
        rel = asset
        safe_visual = {
            "type": "image",
            "path": rel,
...
def parse_script(data: dict[str, Any]) -> EpisodeScript:
    ...
    segments = [_validate_segment(s, index=i) for i, s in enumerate(segments_raw[:MAX_SEGMENTS])]
